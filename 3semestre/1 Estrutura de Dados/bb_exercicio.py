...
def passo_da_busca_binaria(lista,procurando,menor_i,maior_i):
    lista = [10,20,30,40,50,60,66,68,70]
    procurando = 50
    meio = media_arredondada(menor_i, maior_i)
    if procurando == lista[meio]:
        return True
...
'''
t_intervalo_1_7 = 7-1+1
t_intervalo_5_8 = 8-5+1
t_intervalo_10_11 = 11-10+1
t_intervalo_2_1 = 1-2+1
...
def busca_binaria(lista,procurado):
    ...
    while True:
        ...
        if lista[meio] == procurado:
            ...
            if tam_do_intervalo(comeco, fim) > 1:
                if lista[meio] > procurado:
                    fim = meio - 1
                else:
                    comeco = meio + 1
