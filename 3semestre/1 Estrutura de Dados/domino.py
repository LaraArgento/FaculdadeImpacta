...
def peca_valida(peca):
    validos = [1, 2, 3, 4, 5, 6]
    if peca.direita in validos and peca.esquerda in validos:
        return True
...
def encaixa_de_algum_jeito(peca1, peca2):
    if peca1.direita == peca2.esquerda:
        return True
    elif peca2.direita == peca1.esquerda:
        return True
    elif peca2.direita == peca1.direita:
        return True
    elif peca2.esquerda == peca1.esquerda:
        return True
...
, mas não gira a primeira Retorna True se encaixa
'''
...
Um jogo é uma lista de dominós que se encaixam (na posicao atual, sem precisar
girar). Repetindo: todos se encaixam. O dominó 0 se encaixa com o 1,o 1 com o
2, e assim até o domino len(lista)-2, que se encaixa com o len(jogo)-1
...
def jogo_valido(jogo):
    for n in range(1, len(jogo)):
        if (jogo[n-1].direita != jogo[n].esquerda):
            return False
    return True
...
def jogador(jogo, peca):
    ...
    if direita == jogo[-1].direita:
        return 'frente'
    elif esquerda == jogo[-1].direita:
        return 'frente'
    elif direita == jogo[0].esquerda:
        return 'atras'
    elif esquerda == jogo[0].esquerda:
        return 'atras'
    else:
        return False
...
def no_comeco(jogo, peca):
    novo_jogo = jogo
    return novo_jogo.insert(0, peca)
